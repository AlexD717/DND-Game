...
def Shop():
  ...
  for item in shopItems:
    if item.type == "Weapon":
      ...
      print(
          f"This {item.name} is a piece of armor that increases your ac by {item.acMod}, and increases your shield by {item.shieldMod}, costing {item.cost} coins"
      )
  for player in allPlayers:
    #remove this
    player.coins = 10000
...
def SellItems(player: Player):
  if (len(player.weaponList) > 1):
    ...
    if (userChoice != "cancel"):
      ...
      print(
          f"{player.name} sold their {weaponSelling.name} for {coinsGotBack} coins."
      )
      userChoice = AskQuestion("Whould you like to sell another weapon? ",
                               ["yes", "no"])
      if (userChoice == "yes"):
        SellItems(player)
      else:
...
def enemyAttack(enemy, playersInCombat):
  ...
  if (AttackHit(enemy.attackModifier, playerAttacking.acTemp)):
    ...
    if (playerAttacking.healthTemp <= 0):
      ...
      print(
          f"{playerAttacking.name} now has {playerAttacking.healthTemp} health"
      )
      time.sleep(0.5)
  else:
...
def usePotion(player):
  ...
  for potion in potionNames:
    print(potion)
  userChoice = AskQuestion("Which potions would you like to use? ",
                           potionNames)
  ...
  for player in allPlayers:
    ...
    if player.status == "engaged" or player.status == "disengaged":
      ...
      if len(acceptableWeaponList) != 0:
        userChoice = AskQuestion(
            f"Which enemy would {player.name} like to attack? You are currently {player.status}. Alternatively, choose to disengage/re-engage. ",
            enemyNames)
        ...
        if userChoice == "re-engage":
          print(
              f"{player.name} has regained some courage and has decided to re-engage"
          )
          player.status = "engaged"
          continue
        ...
        userChoice = AskQuestion(
            ...
            [weapon.name for weapon in acceptableWeaponList])
        ...
        userChoice = AskQuestion(
            "You have no weapons that can be used currently. Would you like to disengage/re-engage? ",
            ["disengage", "re-engage", "no"])
        if userChoice == "disengage":
          print(f"{player.name} has decided to try and run away")
          player.status = "disengaging"
      ...
      if player.status == "disengaged" and len(player.potionList) > 0:
        userChoice = AskQuestion("Would you like to use a potion? ",
                                 ["yes", "no"])
        if userChoice == "yes":
          player.healthTemp, player.acTemp, player.attackModifierTemp, player.shieldTemp, player.potionList = usePotion(
              player)
...
def AttackHit(attackMod, ac):
  ...
  if (roll == 1) or roll + attackMod < ac:
    ...
    raise Exception(
        "Something went wrong calculating if the attack hit or not")
